...
### WIRE-9 Feedback "reward qualified" toggle has no admin control `[x]` — resolved 2026-07-05
...
- **No contract change** — the endpoint already existed, but had zero test coverage (confirmed by grepping `tests/`), a real gap now that a real client actually calls it. Added `tests/test_feedback_api.py::test_feedback_reward_toggle_round_trips` (submit → toggle on → toggle off, asserting the response body's `reward_qualified` each time) and `::test_feedback_reward_toggle_unknown_id_returns_404`.
- **`web/components/admin-console.tsx`:** added `toggleRewardQualified(key, feedbackId, rewardQualified)`, following the same `adminFetchJson` + loading/error/status pattern as the existing `toggleSuspend`/`saveFlag` handlers. Since the endpoint already returns the full updated `FeedbackItem`, the handler patches that one row into `feedback.items` in place rather than reloading the whole list. Each feedback card in the Feedback tab now shows a "Reward-Qualified" badge (reusing the existing `admin-badge--ok` style) when true, and a "Mark reward-qualified" / "Remove reward flag" button that calls the toggle.
- **Verified end-to-end with Playwright** against the running dev server (no existing test harness covers the admin console's frontend): registered a synthetic user, flipped `is_admin=true` directly in `vinaadi_dev` for that single synthetic row (the admin console's client-side "unlock" key only gates a fallback path — the Next.js `/admin` route itself gates on the session's `is_admin_user()` check before ever rendering the console, so a real admin session was required to reach the UI at all), submitted a synthetic review via the API, then drove the console: unlocked with the real `JOTHIDAM_ADMIN_API_KEY`, opened the Feedback tab, clicked "Mark reward-qualified" — confirmed `PATCH /feedback/{id}/reward -> 200`, the badge appeared immediately, and the button flipped to "Remove reward flag" (screenshotted). Cleaned up afterward: deleted the synthetic feedback row and user from `vinaadi_dev` (feedback.user_id is `SET NULL` on user delete per WIRE-2's FK table, not cascade-deleted, so the feedback row needed an explicit delete); confirmed zero leftover rows.
- **Verified:** `tsc --noEmit` and ESLint clean on `admin-console.tsx`.

### WIRE-10 Admin calibration report has no UI `[x]` — resolved 2026-07-05
...
- **No contract change** — the endpoint already existed and takes no params; this was pure frontend wiring plus closing a real test gap (the endpoint itself had zero test coverage, though the bucket-aggregation math it calls, `build_calibration_report`, was already unit-tested in `tests/reasoning/test_calibration.py`).
- **`web/components/admin-console.tsx`:** added a `calibration` tab (new `AdminTab` union member, new entry in `tabs`, following the Health/Analytics reference pattern the task named). Added `CalibrationBucket`/`CalibrationReport` types mirroring the backend's `CalibrationBucketOut`/`CalibrationReportOut`, a `calibration` state slot, a `loadCalibration(key)` loader (same `adminFetchJson` + status/error pattern as `loadAnalytics`), and a tab-activation `useEffect` matching every other tab. The rendered section shows total logged/open/graded as summary metrics, a "fewer than 30 graded predictions" caveat (lifted directly from the endpoint's own docstring about bucket numbers not being meaningful below n≈30), and two `by_area_band`/`by_dasha_lord` tables reusing the exact `admin-table`/`admin-retention` markup already proven by the adjacent Analytics tab's cohort-retention table.
- **Added backend test coverage** (previously zero) in `tests/test_admin_api.py`: `test_calibration_requires_admin` (non-admin session gets 403) and `test_calibration_report_aggregates_prediction_log` (seeds a chart + 2 graded + 1 open `PredictionLog` row via the ORM, asserts `total_logged`/`total_open`/`total_graded`, per-bucket hit/miss/n/hit_rate math for `CAREER/STRONG`, and that the open `MARRIAGE/LIKELY` row is correctly excluded from both bucket lists). Full `tests/test_admin_api.py`: 10 passed.
- **Verified end-to-end with Playwright** against the running dev server: registered a synthetic user, flipped `is_admin=true` for that single row in `vinaadi_dev` (same admin-console auth gate as WIRE-9 — the Next.js `/admin` route itself requires a real admin session before it will even render the console), unlocked the console, opened the new Calibration tab, and confirmed `GET /admin/calibration -> 200`, the heading/summary metrics/empty-state messaging rendered correctly (screenshotted), and no new console errors. **Scope note:** seeding a *populated* bucket table end-to-end in the dev DB would have required creating a real chart through app-level PII encryption (`birth_latitude`/`birth_longitude` are encrypted columns — a raw-SQL insert attempt failed and was safely rolled back, nothing left behind), which was judged not worth the setup cost here since the populated-render path is a direct array-to-`<table>` map identical to the already-proven Retention-cohort table in the same file, and the bucket math itself is independently covered by the new pytest integration test above. Cleaned up the synthetic user afterward; confirmed zero leftover rows.
- **Verified:** `tsc --noEmit` and ESLint clean on `admin-console.tsx` (one `react/no-unescaped-entities` lint error caught and fixed — an apostrophe in the sub-30 caveat copy).
...
- `GET /charts/{chart_id}/share` (`app/api/charts.py`) `[x]` — resolved 2026-07-05. **Deleted.** Re-confirmed zero callers in web/mobile/shared and zero test coverage (only `share-card`, a distinct route/feature, was tested) before removing `get_chart_share_link` entirely; `_load_chart_and_profile` (the helper it used) is still called by two other handlers in the same file, so it was left in place.
- `GET /charts/{chart_id}/transits/major` — investigated: has zero callers anywhere (web/mobile/shared) **and** zero internal service callers (unlike `/peyarchi` below), but it has a dedicated regression test (`test_major_transits_endpoint_filters_to_major_grahas`) asserting a distinct behavior (filtering to only Guru/Sani/Rahu/Ketu) not covered by `/gochar/current` or `/sani-cycle`. Confirmed intentional, not dead-by-accident — no action taken.
- `GET /charts/{chart_id}/peyarchi` `[x]` — resolved 2026-07-05. Confirmed the doc's claim: `chart_explanation_service.py` and `peyarchi_alert_service.py` call `get_peyarchi_summary()` directly (not the HTTP route), and no web/mobile/shared caller hits this route (only `/peyarchi/upcoming` is client-facing, via `usePersonalData.ts`/`useFamilyData.ts`/`packages/shared/src/api/transits.ts`). Added `include_in_schema=False` to the route decorator in `app/api/transits.py` — kept the route itself (it has direct test coverage, `test_peyarchi_endpoint_returns_upcoming_events`) but hid it from the public-looking OpenAPI surface per the original recommendation.
...
- `app/api/qa.py`'s `POST /qa/validate-golden-case` (compat alias) and `DELETE /qa/regressions/{test_id}` (single-id) `[x]` — resolved 2026-07-05. **Both deleted.** Re-confirmed zero callers in `web`/`mobile`/`packages/shared` (the admin QA tab, actually `dashboard-qa-tab.tsx` — this doc's `admin-console.tsx` citation was stale — uses only `GET /qa/validate`, `GET /qa/regressions`, and the bulk `DELETE /qa/regressions`). `tests/test_qa_api.py` contained exactly one test, and it only covered the now-deleted POST alias, so the file was deleted along with the route; the single-id delete route had no test at all. The now-unused `HTTPException` import was also removed from `app/api/qa.py`. Verified: `python -c "import app.main"` succeeds and route listing confirms only `GET /qa/validate`, `GET /qa/regressions`, `DELETE /qa/regressions` remain.
