...
- `STR-08` **[VARIANT]** Rahu and Ketu have no Bhinnashtakavarga table of their own. When a **transit** is scored for bindu support, Vinaadi substitutes **Saturn's table** for both nodes, and returns a neutral 4 for any graha with no table at all. The code calls this common Thirukanitham practice; **that attribution is currently unsourced in-repo and is exactly what we would like checked.** The `STR-05` karaka-relative rules deliberately do **not** use this proxy — a substitute table is arguably defensible for scoring a transit and meaningless for "the 5th bhava from Guru", so those rules return nothing rather than a proxied answer. **Reviewers: is the Saturn proxy your practice, is another graha used, or should nodes simply be omitted from bindu-based transit scoring?**
...
- `DOS-02` **[TRADITION]** Rahu/Ketu marriage attention checks houses 1, 2, 7, and 8; Sarpa/Kalasarpa-related checks use the Rahu–Ketu arc and whole-sign placement. **[VARIANT] for the Kala Sarpa portion specifically** — schools disagree on whether planets exactly on a node count, whether the Rahu→Ketu direction matters, whether Lagna must fall inside the arc, and what conjunction tolerance applies. Vinaadi's arc test is whole-sign and direction-agnostic.
...
- `PAN-05` **[TRADITION]** Tamil month is derived from the sidereal solar month, not Gregorian month names, with the boundary found by bisecting the Sun's rasi ingress (Sankranti) to the exact instant. **One known divergence, carried explicitly:** `tamil_calendar._MONTH_START_DATE_OVERRIDES` holds a single hardcoded correction — Aavani 1, 2026 is 18 August in the Chennai almanac, where the astronomical rule as implemented gives 17 August. An override means the day-assignment rule does not yet match published practice in every case, and **the general rule is what needs a ruling**, not this one date.
...
- `PAN-09` **[VARIANT]** Abhijit Muhurtham is a fixed **solar noon ± 24 minutes** (48 minutes centred on local apparent noon, tracking the equation of time rather than clock 12:00), and it is **excluded on Wednesday**. "Where applicable" was the previous wording and named neither the window nor the exclusion. Reclassified `[VARIANT]` because the fixed ±24 minutes is the clock-table convention; some traditions scale Abhijit to one fifteenth of the actual daylight span, making it wider in summer and narrower in winter.
...
- `GO-10` **[VARIANT] — reclassified from `[CORE]`, ruling open.** Vinaadi calculates Kandaka Sani from **Lagna**, with Saturn in the 1st, 4th, 7th, or 10th. Kantaka/Kandaka Sani is **not uniform across lineages**: it is variously reckoned from Lagna, from Janma Rasi, or from Arudha Lagna, and the house set is variously 1/4/7/10 or 1/4/8/10. Calling one of those a locked foundational convention overstated it, and this rule changes whether a reader is told they are *currently under* Kandaka Sani, so the marker matters. **Mitigation already in place:** every surface labels it "Kandaka Sani (from Lagna)" / "கண்டக சனி (லக்னம்)", so the reference is disclosed to the reader and never implied to be universal — this is why the rule stays live rather than being disabled. **What is open:** the named Tamil source for the reference and the house set. Logged in `docs/ASTROLOGER_REVIEW_QUEUE.md`; `tests/test_rulebook_invariants.py` pins the code to 1/4/7/10 from Lagna so a doctrine change cannot land without updating this rule too.
...
- `POR-03` **[TRADITION]** Mahendra passes the count set `{4,7,10,13,16,19,22,25}`, counting the girl's nakshatra **from the boy's**; Sthree Deergham uses the lenient accepted threshold of 8 or more from the girl's star to the boy's. Two direction/threshold notes a reviewer should rule on are printed with the tables in the [appendix](VINAADI_RULEBOOK_TABLE_APPENDIX.md): the Mahendra set happens to be closed under `c → 29−c`, so this count direction and the reference spec's opposite one give identical outcomes *for this set only*; and some traditions require Sthree Deergham ≥ 13 rather than ≥ 8.
