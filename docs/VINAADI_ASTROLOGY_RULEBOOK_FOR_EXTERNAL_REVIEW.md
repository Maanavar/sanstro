...
- `YOG-01` **[RETIRED 2026-08-27 — split; do not mark this row]** This was one rule ID covering twenty detector functions and thirty emitted yoga codes. The 2026-08-27 review declined to sign it — *"Twenty independent definitions cannot take one verdict, and 'Raja Yoga' alone has several legitimate classical formulations plus a great many loose modern ones. Do not read a blanket approval into this row."* It is now split into the per-yoga IDs below, each with its own presence test, strength ladder, cancellation set and marker. **The full condition set of every rule is printed in the [table appendix](VINAADI_RULEBOOK_TABLE_APPENDIX.md)**, generated from `app/calculations/yoga_rules.py` and pinned to the emitted yoga codes by `tests/test_yoga_rules.py`, so a new yoga cannot ship without a row and a row cannot describe a yoga the engine does not emit. Mark the rows below; this one is a signpost.
  ...
  > **Where to start.** Three of the rows below are the ones I would want ruled first, because each changes *who sees a yoga* rather than how strongly it reads: `YOG-AD-01` (Adhi fires on **one** benefic where the classical rule wants three), `YOG-DN-01` (a third Dhana condition with no classical parent, and it is the commonest of the three), and `YOG-RY-01` (which of at least four live Raja Yoga formulations we implement). None of the three was changed unilaterally — tightening a presence test removes a yoga from charts that show it today, and that is a doctrine call, not a bug fix.
...
- `YOG-DN-01` **[TRADITION] [PRODUCT]** Dhana Yoga — the 2nd and 11th lords conjunct, in mutual exchange, **or** both standing in a kendra/trikona. The first two are the classical 2nd/11th formulation. **The third has no classical parent** — it is a Vinaadi proxy for "both wealth lords are well placed", and it is much the commonest of the three, so this yoga reads present at PARTIAL on a large share of charts. **Reviewer: should the third condition survive, and should the classical set widen to the 5th and 9th lords?**
...
- `YOG-KD-01` **[TRADITION] [PRODUCT]** Kemadruma — no graha other than Suriyan, the nodes and Chandran itself in the 2nd or 12th from Chandran. Four bhanga are tested; a graha in a kendra from Chandran is a **full** bhanga on its own, which is both classical and necessary — that placement *is* Gaja Kesari, so grading it had one chart reporting Gaja Kesari and Kemadruma as simultaneously active. The other three grade 1 → PARTIAL, 2+ → WEAK, and that grading is Vinaadi's.
...
- `YOG-CH-01` **[TRADITION] [LIMIT]** Guru Chandala — Guru and Rahu in one rasi. **Whole sign, no degree orb**, so a pair 25° apart inside one rasi forms it while a 3° pair across a rasi boundary does not. **Kethu is not tested.** **Reviewer: name your orb and it can be tightened.**
...
- `YOG-AD-01` **[VARIANT] [PRODUCT]** Adhi — **at least one** of Guru, Sukran or Budhan in the 6th, 7th or 8th from Chandran, graded by how many of those three *houses* are covered. **This is the loosest presence test in the yoga set, and it is looser than the classical rule**, which asks for the benefics as a set and grades by how many of the three *benefics* are so placed. Firing on a single benefic makes Adhi Yoga present on most charts. **Reviewer: this is the first row I would want ruled.**
- `YOG-DR-01` **[VARIANT] [PRODUCT]** Daridra — the 11th lord in a dusthana, or scoring under 40 with a malefic sharing its rasi. Daridra yogas are a family; this implements one narrow member of it. The 40 cut-off reads the composite natal graha score (`GO-13` family), not a classical strength.
- `YOG-LK-01` **[TRADITION] [PRODUCT]** Lakshmi — the 9th lord scoring 60 or more and standing in a kendra/trikona, with the Lagna lord scoring 60 or more. **The principle is classical; the two 60s are Vinaadi's** — the source rule reads dignity (own or exaltation sign), not a score.
...
- `YOG-VS-01` **[VARIANT] [PRODUCT]** Vasumati — two or more natural benefics in an upachaya rasi (3/6/10/11) **counted from Chandran only**; the usual statement of the rule allows the Lagna as well. Chandran is in the candidate set but can never satisfy the test, being always the 1st from itself, so the effective set is three grahas.
