...
| 7 — Surfaces | **Numbers-only half built 2026-07-28 (web).** Fortune Alignment, favourable numbers, personal cycle, NUM-58 shortlist, lucky/marriage dates, and the 3 public calculators. Name correction and NUM-34 have **no screen** — their point is a sentence, and the sentences are withheld. Mobile untouched. | `web/components/dashboard-numerology-*.tsx`, `web/app/tools/numerology-calculator/`, `NUMEROLOGY_PHASE7_SURFACES_2026-07-28.md` |
| 6, 8 | Not started. | — |
...
- **Correction 2026-07-29 — the gate was covering two different kinds of text.** "No interpretive prose" was also withholding *Cheiro's own compound titles*, which are a citation of a printed book rather than copy we wrote about a reader. Nothing in "31 is titled *The Recluse* in Cheiro 1935, pp. 126-133" becomes truer or safer after a Tamil review: there is no Tamil in it and no claim about the person reading it. The cost was concrete — the public calculator printed `Compound 31` as a bare integer inside a *collapsed* "How this was worked out" list, between "Adds up to 31" and "Reduced to a single digit", while the sourced title for that exact number sat unused in `numerology_content.py`. The compound **outranks** the root in this system (the first thing `calculations/numerology.py` says), and the product was rendering it as a discarded intermediate step. A fair summary of the result — "this is just adding digits up" — was accurate.
  - Ships ungated, as `NumberReadingOut.compoundTitle` / `compoundTone` / `compoundEchoes` / `compoundSource`, via the new `numerology_content.CompoundCitation`.
  - Stays gated, as `compoundReadingEn` / `compoundReadingTa` through `reviewed_prose`: those are *our* re-renderings of what a number means for a person, drafted by one hand, and the root 1-9 corpus is untouched by this.
  - The citation fields deliberately carry **no `…En`/`…Ta` suffix** — that suffix is exactly what `test_numerology_chart_api.py` scans for, and it means "gated bilingual prose". Mislabelling bibliography as prose is the mistake being undone, so the naming had to stop repeating it.
  - `compoundTone` is mandatory wherever a title renders, enforced by test on both surfaces. Several of Cheiro's titles are alarming read alone — 16 is "The Shattered Citadel", 22 "The Good Man Blinded" — and a title shipped without its register hands the reader his fatalism while withholding our reframing of it. That is standing ruling 3 (the banned fear trade) breached **by omission**, which is the form of it that passes a code review.
- **`letterValues` is on the wire (same date).** `S·3 E·5 N·5 …` for every scored token. "Adds up to 65" is an assertion the reader cannot check; the breakdown is the arithmetic a practitioner writes by hand, and it is the only view in which a *name correction* is legible — a spelling change moves specific letters rather than just some total. Emitted by the backend rather than computed client-side because `CHALDEAN_GROUPS` is **data, not an `A=1..Z=26 mod 9` formula** (no letter carries 9), so a copy in TypeScript would be a second table to get wrong. `reading_from_total` refuses a breakdown that does not sum to its own total — it is shown to users as proof, so the two must not be able to disagree.
