...
- [ ] **AR-5 · WI-07 printed-panchangam sunrise validation** — still the doctrine launch gate: cross-check computed Hindu sunrise + derived fields (Rahu kalam etc.) against 2-3 printed panchangam dates before closing Doctrine §1. (2026-07-16: `tests/test_wi07_sunrise_validation_harness.py` gained two more tiers, neither closes the gate. Tier 2: SwissEph(disc-center,no-refr) vs NOAA/Meeus, agreeing ≤2s, for all 12 date/location cases, sunrise + Rahu Kalam — a regression net only, both legs share this repo's own sunrise definition. Tier 3: live-fetched drikpanchang.com values for the same 12 cases — checked and confirmed NOT usable as printed-panchangam substitutes (it's a live calculator, not a digitized print edition) and NOT exact-match comparable either: DrikPanchang's sunrise is consistently 3-6min earlier than ours across all 12 cases, the signature of it using standard refracted-upper-limb sunrise rather than this repo's disc-center/no-refraction convention — so it's wired as a directional/band check only, same pattern as the pre-existing structural tests. `REFERENCE_CASES` (the actual gate) is still 12x `None`; still needs someone with access to real printed Thirukanitham/Manimekalai/Arcot editions.)
