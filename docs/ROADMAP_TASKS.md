...
## Future Work — Tamil 10-Porutham

Implement the true Tamil 10-Porutham compatibility system as used in Tamil Nadu astrology and the Thirukanitham tradition.

The full specification was captured in **AGENT_WORKBOARD A-07** (completed 2026-06-28, Path B).  The backend implementation lives in `app/calculations/porutham.py` and the tests in `tests/test_astrology_shared_rules.py`.

The 10 poruthams and their weights are:

| # | Porutham | Checks |
|---|---|---|
| 1 | Dina Porutham | Nakshatra count (day count) mod 9 |
| 2 | Gana Porutham | Deva/Manushya/Rakshasa gana |
| 3 | Mahendra Porutham | Nakshatra count from bride to groom |
| 4 | Sthree Dheerga Porutham | Nakshatra count ≥ 13 |
| 5 | Yoni Porutham | Animal symbol compatibility |
| 6 | Rasi Porutham | Rasi lord compatibility |
| 7 | Rasiyathipati Porutham | Rasi count between charts |
| 8 | Vasya Porutham | Mutual attraction by rasi |
| 9 | Rajju Porutham | Birth star's rajju group must differ |
| 10 | Vedha Porutham | Incompatible star pairs must not match |

**Score:** Each porutham is pass/fail — maximum 10/10.  Vedha and Rajju are elimination criteria even when other scores are high.

**Next steps:**
- Web: add a `/porutham` page that displays the 10-check grid with Tamil/English labels.
- Mobile: add porutham screen under Jadhagam section.
- API: expose `GET /api/v1/porutham?bride_id=&groom_id=` returning all 10 results.
