...
- [~] **Status:** CODE-COMPLETE, JHORA-UNVALIDATED (2026-07-16) — this WI's own stop-rule says STOP if JHora is unavailable ("that is how the current wrong formula got in"); JHora was in fact unavailable, but the user explicitly directed proceeding anyway, on condition the direction and co-lord rules be cross-referenced against multiple published sources first (not from memory). That research (documented below) surfaced two independent, concrete bugs beyond what this WI originally scoped, both now fixed in `app/calculations/jaimini_dasha.py`:
  - **Direction bug:** the old code used the Lagna's plain odd/even sign parity. The actual BPHS rule keys off the 9th sign from Lagna's membership in a quadrant-of-3 Savya (forward: Aries-Taurus-Gemini, Libra-Scorpio-Sagittarius) / Apasavya (backward: Cancer-Leo-Virgo, Capricorn-Aquarius-Pisces) grouping — cross-referenced across 3 independent detailed sources (a fully-worked Chara Dasha calculation PDF, jothishi.com, astrobix.com), all agreeing with each other and reconcilable with the primary BPHS/Santhanam text once "pada" there is read as "zodiac quadrant," not "odd/even sign." This is mathematically identical to plain odd/even for movable and fixed lagnas but **disagrees for the four dual-sign lagnas** (Gemini, Virgo, Sagittarius, Pisces) — exactly where the old code was wrong. Two other sources (a BAVA.org page, a Gary Gomes/saptarishisastrology article) state the plain-odd/even simplification instead; given the 3-source worked-example corroboration plus the BPHS reconciliation, the quadrant rule was adopted as the operative default.
  - **Period-length bug:** the old code branched on a movable/fixed/dual axis, which (a) does not match the Savya/Apasavya axis that actually governs forward/backward counting per the same sources, and (b) broke the universal "lord in its own sign = 12 years" special case for fixed and dual rasis (it returned 1 year instead). Both fixed: period length now uses the same Savya/Apasavya grouping for counting direction, with the own-sign case special-cased to 12 uniformly.
  - Also fixed in the same rewrite (dead code, unused/untested, but same bug class): `calculate_chara_antardasha`'s sub-period direction pivoted on the natal Lagna's parity; the correct pivot (confirmed against all 12 mahadashas of the same worked example) is the *running mahadasha rasi's* own 9th-sign rule, not Lagna's.
  - **Scorpio/Aquarius co-lord hierarchy:** implemented as (1) both co-lords conjunct → use the primary; (2) exactly one occupies the sign itself → it wins; (3) neither → whichever has more companion planets in its occupied rasi; (4) still tied → higher degree-in-sign (needs longitudes; falls back to the primary lord if unavailable). One source's wording of step (2) read inverted relative to two others — the majority (occupying wins) reading was used; the discrepancy is noted in the function's docstring rather than silently resolved.
  - **Consumer sweep done in the same change:** `app/api/charts.py`'s `get_chara_dasha` route now threads `planet_longitudes` through (needed for the co-lord degree tiebreak); `packages/shared/src/api/charaDasha.ts` unchanged (no response-shape change — the fix is entirely inside the existing period/rasi/date fields); a static, non-alarmist disclosure caption ("Derived from published BPHS/Jaimini sources — not cross-checked against JHora or another reference chart software") was added to both the web "Classical Timing" → Jaimini Chara Dasha section and the mobile "Jaimini Chara" tab, matching this repo's existing Kalachakra-dasha disclosure convention rather than inventing a new pattern.
  - **Tests:** `tests/test_jaimini_dasha.py` rewritten — golden cases for Savya/Apasavya forward/backward counting, the own-sign-always-12-years fix, all 4 Scorpio/Aquarius co-lord tie-break rungs, and two tests that concretely lock in the dual-sign direction bug fix (Gemini lagna now reverse, Virgo lagna now direct — both the opposite of the old odd/even rule). 20/20 pass.
  - **What is still NOT true:** this has not been cross-checked against JHora or any other reference-software chart, per the WI's own validation strategy — only against published written descriptions of the rule, cross-referenced against each other. The Scorpio/Aquarius step-(2) wording conflict above is a known, documented, unresolved discrepancy between sources (not a guess). Treat as "best available sourced implementation," not "JHora-equivalent."
  - Full backend suite re-verified after this change (and WI-12/OQ-6 in the same session): 1583 passed, 12 skipped (WI-07's own TODO reference slots, expected), 0 failed.
  - Resolved by: Claude (full ownership grant, user-directed override of this WI's own stop-rule, 2026-07-16).
- **Priority:** P1 — launch gate. **Depends on:** WI-09 (same subsystem; land Rahu degree first) — done.
...
- **Rule set implemented:**
  1. **Direction (savya/apasavya):** quadrant-of-3 grouping via the 9th-from-pivot rule (see status note) — NOT plain sign parity.
  2. **Years:** count from the dasha rasi to the rasi occupied by its lord, in that direction, inclusive, **minus one**.
  3. **Own sign:** lord in its own sign → **12 years**, regardless of Savya/Apasavya.
  4. **Scorpio (Mars/Ketu) and Aquarius (Saturn/Rahu):** compute from the **stronger co-lord** per the 4-rung hierarchy above. Implemented — not skipped.
  5. Antardasha direction fixed to pivot on the running mahadasha rasi (dead/unused code path, not wired to any consumer; left unwired since no WI asked for it).
- **Acceptance criteria met:** golden sequence + period-length + co-lord tests pass (20/20, see `tests/test_jaimini_dasha.py`); the old parity-based formula is deleted; the interim "Experimental" label step was skipped by direct user instruction in favor of shipping the sourced rewrite with an honest, permanent (not interim) "not JHora-validated" disclosure instead — see status note for exact wording and placement.
...
- [x] **Status:** DONE (2026-07-16) — Ekadashi (both pakshas, Smarta reckoning, two-consecutive-sunrise dedup per Doctrine §8) + the doctrine starter-set yearly rules (Puthandu, Thai Pongal, Aadi Perukku, Thai Poosam, Vinayagar Chaturthi, Krishna Jayanthi, Deepavali, Karthigai Deepam, Vaikunta Ekadashi naming) added to `_recurring_tithi_festivals`/`get_festivals_for_date`; Sankatahara Chaturthi fixed to fire every Krishna-paksha Chaturthi (not only Tuesday, which is additionally the "Angarki" variant). 2026 parity gate in `tests/test_festivals.py` (35 tests): 20 of 24 hardcoded Ekadashi dates match exactly; 4 explained exceptions (1 genuine Smarta-vs-Vaishnava two-sunrise divergence, 2 pre-existing hardcode date errors the engine corrects, 1 documented tithi-kshaya gap — see OQ-7 below). Hindu-tagged hardcode entries for all newly-algorithmic festivals deleted from `_YEARLY_FESTIVALS[2026]` (govt/Muslim/Christian rows kept, plus a few solar-day-offset entries — Bhogi/Mattu Pongal/Kaanum Pongal — corrected to stay consistent with the verified Pongal date, not made algorithmic). 2027 spot-checked non-empty. Incidental fixes found and applied during this work: (1) `_NAKSHATRA_FESTIVALS`'s "KRITHIGAI" key never matched the canonical "KARTHIGAI" spelling used everywhere else in this codebase — Karthigai Vratam never fired; (2) the name-dedup logic picked whichever category arrived first as primary, which flipped some entries from "hindu" to a govt category once the old hardcode's hindu rows were removed — fixed to prefer "hindu" as primary regardless of arrival order; (3) discovered fetching yesterday's snapshot for the Ekadashi dedup was firing on every single day (not just Ekadashi days), doubling the panchangam cache row count per request — gated to only fetch when today's tithi is actually 11. Scope note: the 24 traditional Puranic per-month Ekadashi names (Shattila, Jaya, Vijaya, ...) are NOT implemented — the engine uses generic paksha-qualified names (except Vaikunta Ekadashi, doctrine's one explicitly named instance); a full lunar-month-keyed name table is an optional follow-up, not required to close the "silent 2027" regression. `app/services/panchangam_events_service.py`/`app/data/panchangam_events_2026.py` (a separate, unrelated hardcode path) intentionally untouched. Resolved by: Claude (full ownership grant, 2026-07-16).
...
- **OQ-1 — Graha yuddha winner rule.** ✅ **Structural bug fixed 2026-07-16** — `chart_strength.detect_planetary_wars` now compares absolute zodiacal longitude (short forward arc, trailing planet loses) for *both* the war-gate and the winner decision, so the sign-boundary inconsistency (29.5° Gemini vs 0.3° Cancer) is gone; the 0/360° Aries seam is handled the same way. This adopts the common absolute-longitude convention as the operative default since it needs no new ephemeris data. **Still genuinely open:** Surya Siddhanta's declination-based alternative ("northern planet wins") is a real classical variant, documented in code as deferred — it needs latitude/declination data not currently threaded into this function's signature. If an astrologer later rules in favor of that convention, this is a signature change, not a one-line swap. Resolved by: Claude (full ownership grant, 2026-07-16) — see `tests/test_chart_strength.py`.
- **OQ-2 — Dinam good-count set.** ✅ **Closed 2026-07-16** — confirmed as final. The 12-count set {2,4,6,8,9,11,13,15,18,20,24,26} is the published Tamil Dinam kutam table, a distinct reckoning from the generic tara-mod-9 cycle; 17/22/27 are correctly excluded, not a gap. No code change (WI-19's comment fix already matches this). Resolved by: Claude (full ownership grant, 2026-07-16) — see `docs/ASTROLOGER_REVIEW_QUEUE.md` Resolved.
...
- **OQ-4 — `EXTENDED_SEVVAI_HOUSES`.** ✅ **Closed 2026-07-16** — removed rather than guessed. No authentic source differentiates an "extended manglik" house list, and the mode was confirmed unreachable from every API/shared/web/mobile surface (unit-test-only). Resolved by: Claude (full ownership grant, 2026-07-16) — see `docs/ASTROLOGER_REVIEW_QUEUE.md` Resolved.
- **OQ-5 — Jeevan/Nethiram unlock (Doctrine §7).** ✅ **Closed 2026-07-16** — astrologer confirmed the values; display restored to all three surfaces, formula/thresholds unchanged. Note the ≥2-printed-sources criterion was *not* recorded in-repo: status is confirmed-by-review, not independently verified (see Doctrine §7 and ASTROLOGER_REVIEW_QUEUE). C-2 (harsh label copy) also ✅ **closed 2026-07-16** — classical terms kept verbatim, explanatory gloss added instead of softening (see ASTROLOGER_REVIEW_QUEUE Resolved).
- **OQ-6 — Murthi golden dates (supports WI-08/WI-07 validation).** ⚠ **Partially closed 2026-07-16** — 2 regression tests added (`tests/test_transits_calculations.py::test_ezharai_sani_murthi_self_computed_kumbham_ingress_2023`, `..._makaram_ingress_2020`) locking the murthi grade this repo's own ephemeris pipeline (`find_saturn_ingress_jd` + real Moon rasi at that instant) produces for 2 real historical Saturn ingresses (Kumbham ~2023, Makaram ~2020), across 3 synthetic janma rasis each. Explicitly labeled self-computed, not printed-source-verified — **still need** 3–4 independently-known Saturn-ingress murthi verdicts from a printed panchangam to actually validate this pipeline against an external source; OQ-6 stays open for that. Resolved (partially) by: Claude (full ownership grant, 2026-07-16).
- **OQ-7 — Ekadashi tithi-kshaya handling (new, WI-12).** The pure udaya-tithi rule this whole module uses can't observe an Ekadashi whose tithi is entirely elided between two sunrises (verified: 2026-07-10, "Yogini Ekadashi" in the old hardcode — tithi 11 governs neither 07-10's nor 07-11's sunrise). This is a general characteristic affecting every tithi-anchored festival in this codebase equally (17 such skip events occur across all tithis in 2026, roughly one every three weeks), not something new to Ekadashi, and Doctrine §8 doesn't specify a kshaya rule. Need: which adjoining day (or a combined/reduced observance) classical practice assigns in this case — options include "the day Dashami ends" vs "the day Dvadashi begins" vs a merged Ekadashi-Dvadashi observance, and whether this should be handled generally (all tithi festivals) or just for Ekadashi given its outsized importance. See `tests/test_festivals.py::test_yogini_ekadashi_kshaya_tithi_gap_is_a_known_limitation`.
...
- [x] WI-12 Festival rules engine + algorithmic Smarta Ekadashi; 2026 parity gate; 2027 asserted non-empty — done 2026-07-16 (see status note above; OQ-7 tracks the one documented tithi-kshaya gap)
...
- [ ] OQ-3 Pushkara cross-check (needs printed source) · OQ-6 Murthi golden dates (partially closed 2026-07-16 — self-computed regression lock added, printed-source cross-check still needed) · OQ-7 Ekadashi tithi-kshaya handling (new, WI-12)
- [x] ~~OQ-1 Graha yuddha rule~~ — structural bug fixed 2026-07-16 (absolute-longitude comparison; Surya Siddhanta declination variant documented as deferred, needs latitude data not yet threaded through)
- [x] ~~OQ-2 Dinam set~~ — closed 2026-07-16
- [x] ~~OQ-4 Extended Sevvai set~~ — closed 2026-07-16 (dead mode removed, not a guessed table)
- [x] ~~OQ-5 Jeevan/Nethiram tables~~ — closed 2026-07-16
