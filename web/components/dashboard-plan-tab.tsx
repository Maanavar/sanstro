...
export const verdictColor = (v: string) => (v === "FAVOURABLE" ? "var(--chart-d9-active)" : v === "CAUTION" ? "var(--planet-saturn)" : "var(--panel-brand)");
export const verdictKey = (v: string) => (v === "FAVOURABLE" ? "verdict_favourable" : v === "CAUTION" ? "verdict_caution" : "verdict_neutral");
export const strengthColor = (s: string) => (s === "STRONG" ? "var(--chart-d9-active)" : s === "WEAK" ? "var(--planet-saturn)" : "var(--panel-brand)");
...
export function PlanWhatIfPanel({
  lang,
  whatIfScenario,
  whatIfDate,
  whatIfResult,
  ...
  return (
    <Surface title={t("whatif_panel_title", lang)}>
      <div className="surface__body">
        ...
        <div style={{ padding: "var(--space-2) var(--space-3)", borderRadius: "var(--radius-sm)", background: "var(--color-info-light)", border: "1px solid var(--color-earth-mid)", marginBottom: "var(--space-3_5)" }}>
        ...
        {whatIfResult && (() => {
          ...
          return (
            <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
              ...
              <div style={{ padding: "var(--space-2_5) var(--space-3)", borderRadius: "var(--radius-sm)", background: "var(--chart-d9-active-bg)", border: "1px solid var(--cl-sage-20)" }}>
              ...
              <div style={{ padding: "var(--space-2_5) var(--space-3)", borderRadius: "var(--radius-sm)", background: "var(--color-warm-cream)", border: "1px solid var(--cl-brand-ring-md)" }}>
              ...
              <div style={{ padding: "var(--space-2_5) var(--space-3)", borderRadius: "var(--radius-sm)", background: "var(--color-warm-cream)", border: "1px solid var(--cl-brand-25)" }}>
...
export function PlanDecisionsPanel({ lang, chartId, mode = "BALANCED" }: { lang: Lang; chartId: string; mode?: "BEGINNER" | "BALANCED" | "TRADITIONAL" }) {
  if (mode === "BEGINNER") return null;
  return (
    <Surface title={t("decision_panel_title", lang)}>
      <div className="surface__body">
        <div style={{ margin: "0 0 var(--space-3_5)", padding: "var(--space-2_5) var(--space-3_5)", borderRadius: "var(--radius-sm)", background: "var(--color-info-light)", border: "1px solid var(--color-earth-ring)" }}>
...
export function PlanMuhurtaPanel({ lang, chartId }: { lang: Lang; chartId: string }) {
    ...
    <>
      ...
      <div style={{ borderRadius: "var(--radius-md)", border: `1px solid ${W.borderLt}`, background: W.surface, padding: "var(--space-3_5) var(--space-4)", marginBottom: "var(--space-3)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", flexWrap: "wrap", marginBottom: "var(--space-2)" }}>
          ...
          <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", padding: "2px 9px", borderRadius: "var(--radius-pill)", background: "var(--chart-d9-active-bg)", border: "1px solid var(--cl-sage-border)", color: "var(--color-sage-dark)", fontSize: "0.6875rem", fontWeight: 700 }}>
      ...
      <Surface title={lang === "ta" ? "படி 1 — சிறந்த நாட்கள் கண்டறிய (விரைவு மாத கண்ணோட்டம்)" : "Step 1 — Find Best Dates (quick month scan)"}>
        <div className="surface__body">
          ...
          <p style={{ margin: "0 0 var(--space-2_5)", fontSize: "0.72rem", color: W.muted, lineHeight: 1.45, padding: "var(--space-1_5) var(--space-2_5)", borderRadius: "var(--radius-sm)", background: "var(--color-info-light)", border: `1px solid var(--color-earth-mid)` }}>
            {lang === "ta"
          ...
          {activityTimingResult && (
            <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
              ...
              {activityTimingResult.topDates.map((item, i) => {
                ...
                const alignBg = item.alignment === "SUPPORTS" ? "var(--cl-sage-soft)" : item.alignment === "CAUTION" ? "var(--cl-rust-fill)" : "var(--ring-brand)";
                ...
                return (
                  <div
                    ...
                    key={item.dateLocal}
                    style={{ padding: "var(--space-3) var(--space-4)", borderRadius: "var(--radius-md)", background: isSelected ? "var(--chart-d9-active-bg)" : W.card, border: `1.5px solid ${isSelected ? "var(--cl-sage-50)" : W.borderLt}`, cursor: "pointer", transition: "all 0.12s" }}
                    onClick={() => {
...
export function DashboardPlanTab({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4_5)", color: W.inkMid, fontFamily: "var(--font-body)" }}>
      ...
      >
        ...
        {goals.length > 0 && (
          <div style={{ display: "flex", gap: "var(--space-1_5)", flexWrap: "wrap", alignItems: "center" }}>
            ...
            {goals.map((g) => (
              <span key={g.goalId} style={{ fontSize: "0.75rem", padding: "var(--space-0_75) var(--space-2_5)", borderRadius: "var(--radius-pill)", background: "var(--chart-d1-lagna-bg)", border: "1px solid var(--cl-brand-28)", color: "var(--planet-lagna)", fontWeight: 600 }}>
          ...
          <div className="surface__body">
            ...
            {goals.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--space-2)", marginBottom: "var(--space-3_5)" }}>
                {goals.map((g) => (
                  <div key={g.goalId} style={{ display: "flex", alignItems: "center", gap: "var(--space-1_5)", padding: "var(--space-1) var(--space-2_5)", borderRadius: "var(--radius-pill)", background: "var(--chart-d1-lagna-bg)", border: "1px solid var(--cl-brand-edge)", fontSize: "0.875rem", color: "var(--planet-lagna)", fontWeight: 600 }}>
                    <span>
                      {GOAL_OPTIONS.find(([v]) => v === normalizeGoalType(g.goalType))?.[1]
                        ? t(GOAL_OPTIONS.find(([v]) => v === normalizeGoalType(g.goalType))![1], lang)
                        : g.goalType}
            ...
            {goals.length > 0 && (
              <div style={{ padding: "var(--space-2_5) var(--space-3)", borderRadius: "var(--radius-sm)", background: "var(--chart-d9-active-bg)", border: "1px solid var(--cl-sage-20)", marginBottom: "var(--space-3)" }}>
            ...
            {goals.length > 0 && (
              <div style={{ padding: "var(--space-2) var(--space-3)", borderRadius: "var(--radius-sm)", background: "var(--color-info-light)", border: "1px solid var(--color-earth-mid)", marginBottom: "var(--space-3)" }}>
                <p style={{ margin: 0, fontSize: "0.75rem", color: W.muted, lineHeight: 1.5 }}>
                  <strong style={{ color: W.inkMid }}>
      ...
      {planSubTab === "events" && <PlanEventsPanel lang={lang} chartId={chartId} />}
      ...
      {planSubTab === "decisions" && <PlanDecisionsPanel lang={lang} chartId={chartId} mode={mode} />}

      {planSubTab === "muhurta" && <PlanMuhurtaPanel lang={lang} chartId={chartId} />}
