    ...
    <div className="tab-section">
      ...
      {result && (
        <div style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap" }}>
          <div className="card" style={{ padding: "var(--space-4) var(--space-6)", flex: 1, minWidth: "140px", textAlign: "center" }}>
            ...
            <p style={{ margin: "var(--space-0_5) 0 0", fontSize: "0.75rem", color: "rgba(255,255,255,0.45)", letterSpacing: "0.05em" }}>{t("qa_passed", lang)}</p>
          </div>
          <div className="card" style={{ padding: "var(--space-4) var(--space-6)", flex: 1, minWidth: "140px", textAlign: "center" }}>
            <p style={{ margin: 0, fontSize: "2rem", fontWeight: 800, color: result.total_failed > 0 ? "#f87171" : "#4ade80" }}>{result.total_failed}</p>
            <p style={{ margin: "var(--space-0_5) 0 0", fontSize: "0.75rem", color: "rgba(255,255,255,0.45)", letterSpacing: "0.05em" }}>{t("qa_failed", lang)}</p>
          </div>
          <div className="card" style={{ padding: "var(--space-4) var(--space-6)", flex: 1, minWidth: "140px", textAlign: "center" }}>
            <p style={{ margin: 0, fontSize: "2rem", fontWeight: 800, color: "rgba(255,255,255,0.8)" }}>{result.total_passed + result.total_failed}</p>
            <p style={{ margin: "var(--space-0_5) 0 0", fontSize: "0.75rem", color: "rgba(255,255,255,0.45)", letterSpacing: "0.05em" }}>{t("qa_total", lang)}</p>
          </div>
          {allPass && (
      ...
      {!result && !running && (
        <div className="card" style={{ padding: "var(--space-6)", textAlign: "center" }}>
          <p style={{ margin: 0, color: "rgba(255,255,255,0.4)", fontSize: "0.875rem" }}>{t("qa_never_run", lang)}</p>
        </div>
      )}
      ...
      {result && result.modules.map((mod) => (
        <div key={mod.module} className="card" style={{ padding: 0, overflow: "hidden" }}>
          ...
          >
            <span style={{
              fontSize: "0.75rem", fontWeight: 700, padding: "var(--space-0_5) var(--space-2)", borderRadius: "var(--radius-pill)",
            ...
            <span style={{ fontWeight: 600, fontSize: "0.875rem", color: "rgba(255,255,255,0.85)", fontFamily: "monospace" }}>{mod.module}</span>
                ...
                <thead>
                  <tr style={{ background: "rgba(255,255,255,0.03)" }}>
                    {[t("qa_test_id", lang), t("qa_description", lang), t("qa_expected", lang), t("qa_actual", lang), t("qa_status", lang)].map((h) => (
                      <th key={h} style={{ padding: "var(--space-1_5) var(--space-3)", textAlign: "left", fontWeight: 600, color: "rgba(255,255,255,0.4)", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {mod.cases.map((c) => (
                    <tr key={c.test_id} style={{ borderTop: "1px solid rgba(255,255,255,0.04)", background: c.passed ? "transparent" : "rgba(239,68,68,0.05)" }}>
                      <td style={{ padding: "var(--space-1_5) var(--space-3)", fontFamily: "monospace", color: "rgba(255,255,255,0.6)", whiteSpace: "nowrap" }}>{c.test_id}</td>
                      <td style={{ padding: "var(--space-1_5) var(--space-3)", color: "rgba(255,255,255,0.75)", maxWidth: "320px" }}>{c.description}</td>
                      <td style={{ padding: "var(--space-1_5) var(--space-3)", fontFamily: "monospace", color: "rgba(255,255,255,0.5)", whiteSpace: "nowrap" }}>{String(c.expected)}</td>
                      <td style={{ padding: "var(--space-1_5) var(--space-3)", fontFamily: "monospace", color: c.passed ? "rgba(255,255,255,0.5)" : "#f87171", whiteSpace: "nowrap" }}>{String(c.actual)}</td>
                      <td style={{ padding: "var(--space-1_5) var(--space-3)", whiteSpace: "nowrap" }}>
                        <span style={{
                          fontSize: "0.625rem", fontWeight: 700, padding: "var(--space-0_5) var(--space-1_5)", borderRadius: "var(--radius-pill)",
      ...
      {regressions && (
        <Surface title={t("qa_regressions", lang)}>
          ...
          {regressions.total_stored === 0 ? (
            <p style={{ fontSize: "0.875rem", color: "rgba(255,255,255,0.4)", margin: 0 }}>{t("qa_no_regressions", lang)}</p>
          ) : (
              ...
              {regressions.failures.map((f) => (
                <div key={f.test_id} style={{ borderRadius: "var(--radius-sm)", border: "1px solid rgba(248,113,113,0.25)", background: "rgba(239,68,68,0.06)", padding: "var(--space-2_5) var(--space-3_5)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2_5)", flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.5)", fontFamily: "monospace" }}>{f.module}</span>
                    <span style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.3)", marginLeft: "auto" }}>×{f.occurrences}</span>
                  </div>
                  <p style={{ margin: "var(--space-1) 0 0", fontSize: "0.875rem", color: "rgba(255,255,255,0.65)" }}>{f.description}</p>
                  <p style={{ margin: "var(--space-0_5) 0 0", fontSize: "0.75rem", color: "rgba(255,255,255,0.35)" }}>
