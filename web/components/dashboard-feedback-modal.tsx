  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="card" style={{ width: "min(480px, 100%)", padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        {done ? (
          <p style={{ textAlign: "center", color: "#4ade80", padding: "24px 0" }}>✓ {t("feedback_thanks", lang)}</p>
        ) : (
          <>
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {(["bug", "calculation", "suggestion", "other"] as const).map((c) => (
                  <button key={c} type="button" className={`button button--ghost${category === c ? " button--active" : ""}`} style={{ fontSize: "0.78rem", padding: "4px 12px", opacity: category === c ? 1 : 0.5 }} onClick={() => setCategory(c)}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              ...
              <div style={{ display: "flex", gap: "6px" }}>
                {[1, 2, 3, 4, 5].map((n) => (
                  <button key={n} type="button" style={{ fontSize: "1.3rem", background: "none", border: "none", cursor: "pointer", opacity: rating !== null ? (n <= rating ? 1 : 0.3) : 0.35, filter: rating !== null && n <= rating ? "none" : "grayscale(1)" }} onClick={() => setRating(n === rating ? null : n)}>
                    ★
                  </button>
                ))}
              </div>
