#!/usr/bin/env node
/**
 * Dashboard experience audit — the measuring instrument behind
 * docs/DASHBOARD_EXPERIENCE_AUDIT_2026-09-17.md. Every gate below carries the
 * DXA id it verifies, so "done" on an item is a number here, not a claim.
 *
 ...
 *        [--phases load,tabs,today,hover,overlays,reduced,light,phone] [--email <existing e2e account>]
 ...
 * depend on timing (DXA-06) are reported as INFO unless --prod is passed.
 */
...
const ALL_PHASES = ["load", "tabs", "today", "hover", "overlays", "reduced", "light", "phone"];
const PHASES = new Set(String(arg("phases", ALL_PHASES.join(","))).split(",").map((s) => s.trim()).filter(Boolean));
const PROD = arg("prod", false) === true;
const REUSE_EMAIL = arg("email", null);
const PASSWORD = String(arg("password", "UxAudit!Test123"));
const EMAIL = typeof REUSE_EMAIL === "string" ? REUSE_EMAIL : `ux-audit-${Date.now()}@e2e.test`;
const CSRF = { "X-Vinaadi-CSRF": "1" };

const T0 = Date.now();
const metrics = { base: BASE, email: EMAIL, phases: [...PHASES], startedAt: new Date().toISOString(), notes: [] };
...
async function login(api) {
  for (let i = 0; i < 12; i++) {
    const r = await api.post("/api/backend/api/v1/auth/login", { data: { email: EMAIL, password: PASSWORD }, headers: CSRF });
    if (r.ok()) return;
    await sleep(500);
  }
...
async function bootstrap(api) {
  if (typeof REUSE_EMAIL === "string") {
    await login(api);
    return;
  }
  const reg = await api.post("/api/backend/api/v1/auth/register", { data: { email: EMAIL, password: PASSWORD, consentGiven: true }, headers: CSRF });
  if (!reg.ok()) throw new Error(`register failed: ${reg.status()} ${await reg.text()}`);
  await login(api); // register commits in dependency teardown — login is retried
...
async function shot(page, name) {
  await page.screenshot({ path: path.join(OUT, `${name}.png`) }).catch((e) => log(`shot ${name}: ${e.message}`));
}

async function sectionShots(page, name, max) {
 ...
 * works at all (DXA-41).
 */
async function overlay(page, name, open, selector, trigger) {
  const result = { name };
  try {
    await open();
    result.enter = await page.evaluate(ENTER_PROBE, selector);
    await sleep(400);
    await shot(page, `overlay-${name}`);
...
async function newContext(browser, opts) {
  ...
  const ctx = await browser.newContext({ baseURL: BASE, colorScheme: theme, ...rest });
...
try {
  {
    ...
    await bootstrap(boot.request);
    await boot.close();
    log(`account ready: ${EMAIL}`);
  }

  const { ctx: dark, page } = await newContext(browser, { viewport: { width: 1440, height: 900 } });
  ...
  if (PHASES.has("tabs")) {
    ...
    for (const tab of TABS.slice(1)) {
      ...
      await shot(page, `tab-${tab.slug}`);
      const sections = await sectionShots(page, `tab-${tab.slug}`, tab.id === "family" ? 10 : 5);
  ...
  if (PHASES.has("today")) {
    ...
    metrics.today = { sections: await sectionShots(page, "today", 8) };
    await page.evaluate(() => window.scrollTo(0, 0));
    await sleep(400);
    ...
    await settle(page, 800);
    await shot(page, "today-next-day");
    save();
  }
  ...
  if (PHASES.has("overlays")) {
    ...
    metrics.overlays.push(await overlay(page, "more-menu", clickOpen("button.cd-tab--more"), ".cd-dropdown--nav", "button.cd-tab--more"));
    metrics.overlays.push(await overlay(page, "notifications", clickOpen(".cd-topbar__right .cd-icon-btn"), ".cd-alerts-popover", ".cd-topbar__right .cd-icon-btn"));
    metrics.overlays.push(await overlay(page, "account-menu", clickOpen(".cd-topbar .cd-avatar"), ".cd-dropdown:not(.cd-dropdown--nav)", ".cd-topbar .cd-avatar"));
    metrics.overlays.push(await overlay(page, "ask-vinaadi", clickOpen(".cd-ask-search"), '[role="dialog"]', null));
    ...
    if (await monthly.isVisible().catch(() => false)) {
      await monthly.click();
      await settle(page, 1200);
      metrics.overlays.push(await overlay(page, "day-drawer", () => page.locator("button.nova-cal-cell").nth(10).click({ timeout: 8000 }), ".drawer", null));
    }
    save();
  }
  await dark.close();
  ...
  if (PHASES.has("reduced")) {
    log("reduced motion");
    const { ctx, page: rp } = await newContext(browser, { viewport: { width: 1440, height: 900 }, reducedMotion: "reduce" });
  ...
  if (PHASES.has("phone")) {
    log("phone 390x844");
    const { ctx, page: pp } = await newContext(browser, { viewport: { width: 390, height: 844 }, deviceScaleFactor: 2, isMobile: true, hasTouch: true });
    metrics.phone = {};
    for (const slug of ["today", "calendar", "family", "goals", "tools"]) {
      await pp.goto(`/dashboard/${slug}`);
      await settle(pp, 1200);
      await shot(pp, `phone-${slug}`);
      await pp.evaluate(() => window.scrollTo(0, 1500));
      await sleep(700);
      await shot(pp, `phone-${slug}-scrolled`);
  ...
  metrics.gates = computeGates(metrics);
  save();
  await browser.close();
  printGates(metrics.gates);
  log(`done → ${OUT}`);
}
...
function printGates(gates) {
  ...
  for (const x of gates) {
    ...
    console.log(`${x.result.padEnd(5)} ${x.id.padEnd(7)} ${x.check.padEnd(w)}  ${value}`);
  }
  const fails = gates.filter((x) => x.result === "FAIL").length;
  console.log(`\n${fails} failing, ${gates.filter((x) => x.result === "PASS").length} passing, ${gates.filter((x) => x.result === "INFO").length} info\n`);
}
