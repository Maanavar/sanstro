...
import { PARIHARAM_MARRIAGE, PARIHARAM_MARRIAGE_FAQ, mt } from "@/lib/marketing-i18n";
...
export async function ThirumanaThadaiContent() {
  const lang = await getServerLang();
  const d = PARIHARAM_MARRIAGE;
  ...
  return (
    <div className="clarity-shell">
      ...
      <main>
        ...
        <ContextualSignupCta variant="marriage-pariharam" />

        <section className="cl-band cl-band--alt">
          <div className="cl-container">
            <div style={{ marginTop: "2.5rem" }}>
              <FaqSection items={PARIHARAM_MARRIAGE_FAQ} />
            </div>
            ...
            <div className="cl-pub-related" style={{ marginTop: "2rem" }}>
              ...
              <div className="cl-pub-related-links">
                <Link href="/dosham/sevvai-dosham" className="cl-pub-related-link">{lang === "en" ? "Sevvai (Mangal) Dosham →" : "செவ்வாய் தோஷம் →"}</Link>
                <Link href="/tools/marriage-porutham-calculator" className="cl-pub-related-link">{lang === "en" ? "Marriage Porutham Calculator →" : "திருமணப் பொருத்தம் பார் →"}</Link>
                <Link href="/temples" className="cl-pub-related-link">{lang === "en" ? "Temples & their power →" : "கோயில்களும் சக்தியும் →"}</Link>
              </div>
            </div>
