...
import { PARIHARAM_RAHU_KETU, PARIHARAM_RAHU_KETU_FAQ, mt } from "@/lib/marketing-i18n";
...
export async function RahuKetuPariharamContent() {
  const lang = await getServerLang();
  const d = PARIHARAM_RAHU_KETU;
        ...
        <section className="cl-band cl-band--alt">
          <div className="cl-container">
            <div style={{ marginBottom: "2.5rem" }}>
              <FaqSection items={PARIHARAM_RAHU_KETU_FAQ} />
            </div>
            ...
            <div className="cl-pub-related" style={{ marginTop: "2rem" }}>
              ...
              <div className="cl-pub-related-links">
                <Link href="/pariharam/naga-dosha-pariharam" className="cl-pub-related-link">
                  {lang === "en" ? "Naga Dosham Pariharam →" : "நாக தோஷ பரிகாரம் →"}
                </Link>
                <Link href="/temples/thirunageswaram" className="cl-pub-related-link">
                  {lang === "en" ? "Thirunageswaram — Rahu temple →" : "திருநாகேஸ்வரம் — ராகு கோயில் →"}
                </Link>
                <Link href="/temples/keezhaperumpallam" className="cl-pub-related-link">
                  {lang === "en" ? "Keezhaperumpallam — Ketu temple →" : "கீழப்பெரும்பள்ளம் — கேது கோயில் →"}
                </Link>
              </div>
            </div>
