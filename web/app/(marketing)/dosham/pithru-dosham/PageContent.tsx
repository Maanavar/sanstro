...
import { DOSHAM_PITHRU, DOSHAM_PITHRU_FAQ, mt } from "@/lib/marketing-i18n";
...
export async function PithruDoshamContent() {
  const lang = await getServerLang();
  const d = DOSHAM_PITHRU;
  ...
  return (
    <div className="clarity-shell">
      ...
      <main>
        ...
        <section className="cl-band cl-band--alt">
          <div className="cl-container">
            <FaqSection items={DOSHAM_PITHRU_FAQ} />
            ...
            <div className="cl-pub-related" style={{ marginTop: "2rem" }}>
              ...
              <div className="cl-pub-related-links">
                <Link href="/dosham/naga-sarpa-dosham" className="cl-pub-related-link">{lang === "en" ? "Naga / Sarpa Dosham →" : "நாக / சர்ப்ப தோஷம் →"}</Link>
                <Link href="/temples" className="cl-pub-related-link">{lang === "en" ? "Rameswaram & Navagraha temples →" : "ராமேஸ்வரம் & நவகிரக கோயில்கள் →"}</Link>
                <Link href="/dosham" className="cl-pub-related-link">{lang === "en" ? "All doshams →" : "அனைத்து தோஷங்கள் →"}</Link>
              </div>
            </div>
