...
def test_whatif_overall_verdict_thresholds() -> None:
    # Legacy path (reasoning_gate off): band is None, thresholds unchanged.
    score, verdict, band = ws._overall_verdict(70, 70, 70, 70)
    assert score >= 62
    assert verdict == "FAVOURABLE"
    assert band is None

    score, verdict, band = ws._overall_verdict(40, 80, 80, 70)
    assert score >= 45
    assert verdict == "NEUTRAL"
    assert band is None

    score, verdict, band = ws._overall_verdict(30, 30, 30, 30)
