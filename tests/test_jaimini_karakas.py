...
def test_atmakaraka_ranking_against_t003_reference_chart() -> None:
    # Degree-within-sign for each candidate (hand-derived from the longitudes
    # above): VENUS 25.97 > RAHU 22.79 > MERCURY 19.35 > MARS 19.08 >
    # JUPITER 17.96 > SATURN 1.08 > SUN 0.76 > MOON 0.01.
...
def test_rahu_degree_counted_forward_not_reversed() -> None:
    # Documented convention: Rahu's degree-in-sign is taken as-is (0-30),
    # not reversed. A Rahu near the end of its sign should outrank a Rahu
    # near the start when all else is held constant.
    low = compute_char_karakas({"RAHU": 2.0, "SUN": 15.0})
    high = compute_char_karakas({"RAHU": 28.0, "SUN": 15.0})
    assert low["ATMAKARAKA"] == "SUN"
    assert high["ATMAKARAKA"] == "RAHU"


def test_tie_break_uses_classical_dignity_order() -> None:
    # SUN and RAHU at the exact same degree-in-sign: Sun (earlier in
    # classical dignity order) keeps the higher karaka.
    karakas = compute_char_karakas({"SUN": 10.0, "RAHU": 10.0})
