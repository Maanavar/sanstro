...
def test_legacy_path_unchanged_and_band_free():
    overall, verdict, band = _overall_verdict(70, 70, 70, 70)
    assert band is None
    assert overall == 70
...
def test_legacy_soft_floor_still_applies():
    ...
    overall, verdict, band = _overall_verdict(45, 90, 90, 90)
    assert band is None
...
def test_blocked_natal_cannot_be_lifted_by_any_timing():
    for dasha, gochar, panch in itertools.product((0, 50, 100), repeat=3):
        overall, verdict, band = _overall_verdict(
...
def test_pass_gate_favourable_when_timing_aligned():
    overall, verdict, band = _overall_verdict(70, 80, 75, 70, use_reasoning_gate=True)
...
def test_weak_gate_caps_band_at_likely():
    overall, verdict, band = _overall_verdict(50, 80, 75, 70, use_reasoning_gate=True)
    assert band == "LIKELY"  # STRONG timing capped by WEAK gate
    assert verdict == "FAVOURABLE"


def test_gate_thresholds_boundary():
    _, _, band_weak = _overall_verdict(42, 80, 75, 70, use_reasoning_gate=True)
    _, _, band_blocked = _overall_verdict(41, 80, 75, 70, use_reasoning_gate=True)
    assert band_weak != "BLOCKED"
    assert band_blocked == "BLOCKED"


def test_pass_gate_with_poor_timing_is_caution_not_blocked():
    # Promised but not now (the Phase 3 reading, already visible in the band).
    overall, verdict, band = _overall_verdict(70, 20, 20, 40, use_reasoning_gate=True)
    assert band == "WEAK"
    assert verdict == "CAUTION"
